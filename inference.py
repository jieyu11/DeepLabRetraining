...
class DeepLabV3:
    ...
    def segment(self, image_path, threshold=0.5):
        ...
        Arguments:
            ...
            threshold: (float), threshold of each pixel to mask it as person.
        
        return mask (2D np.array)
        """
        assert os.path.exists(image_path), "%s not found!" % image_path
        img = cv2.imread(image_path)
        return self.segment_image(img, threshold)
    
    def segment_image(self, img, threshold=0.5):
        """
        Make 1 image segmentation inference with the given model, while
        ...
        Arguments:
            ...
            threshold: (float), threshold in [0, 1], default is 0.5
        ...
        if rawimgshape != (self.height, self.width):
        ...
        logger.debug("raw image shape: %s" % str(img.shape))
        with torch.no_grad():
